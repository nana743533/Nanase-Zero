...
const int global_place[n_board_idx][hw]{
    ...
    {5, 14, 23, -1, -1, -1, -1, -1},  {4, 13, 22, 31, -1, -1, -1, -1},
    {3, 12, 21, 30, 39, -1, -1, -1},  {2, 11, 20, 29, 38, 47, -1, -1},
    {1, 10, 19, 28, 37, 46, 55, -1},  {0, 9, 18, 27, 36, 45, 54, 63},
    {8, 17, 26, 35, 44, 53, 62, -1},  {16, 25, 34, 43, 52, 61, -1, -1},
    {24, 33, 42, 51, 60, -1, -1, -1}, {32, 41, 50, 59, -1, -1, -1, -1},
    {40, 49, 58, -1, -1, -1, -1, -1}, {2, 9, 16, -1, -1, -1, -1, -1},
    {3, 10, 17, 24, -1, -1, -1, -1},  {4, 11, 18, 25, 32, -1, -1, -1},
    {5, 12, 19, 26, 33, 40, -1, -1},  {6, 13, 20, 27, 34, 41, 48, -1},
    {7, 14, 21, 28, 35, 42, 49, 56},  {15, 22, 29, 36, 43, 50, 57, -1},
    {23, 30, 37, 44, 51, 58, -1, -1}, {31, 38, 45, 52, 59, -1, -1, -1},
...
int move_arr[3][n_line][hw][2];
int legal_arr[3][n_line][hw];
int place_included[hw2][4];
int local_place[hw2][4];
int flip_arr[3][n_line][hw];
int put_arr[3][n_line][hw];
...
inline int move_line_half(const int p, const int o, const int place,
                          const int k) {
  int mask;
  int res = 0;
  int pt = 1 << (hw - 1 - place);
...
void init_board() {
  ...
  for (int idx = 0; idx < n_line; ++idx) {
    int b = create_one_color(idx, black); // Use macro
    int w = create_one_color(idx, white); // Use macro
    ...
    for (int place = 0; place < hw; ++place) {
      ...
      if (move_arr[black][idx][place][0] == 0 &&
          move_arr[black][idx][place][1] == 0)
      ...
      if (move_arr[white][idx][place][0] == 0 &&
          move_arr[white][idx][place][1] == 0)
...
public:
  ...
  struct hash {
    ...
    size_t operator()(const board &b) const {
      return b.board_idx[0] + b.board_idx[1] * 17 + b.board_idx[2] * 289 +
             b.board_idx[3] * 4913 + b.board_idx[4] * 83521 +
             b.board_idx[5] * 1419857 + b.board_idx[6] * 24137549 +
  ...
  inline void trans_arr(int arr[]) {
    ...
    for (int row = 0; row < hw; ++row) {
      ...
      for (int col = 0; col < hw; ++col) {
        int trit =
            (line / pow3[hw - 1 - col]) % 3; // その行の col 列目の値 (0,1,2)
        arr[row * 8 + col] = trit;
      }
    }
  }
    ...
    for (int i = 0; i < n_board_idx; i++) {
      ...
      for (int j = 0; j < hw; j++) {
        if (global_place[i][j] == -1)
          arr_n = vacant; // Padding with Vacant
        else
  ...
  inline board move(const int g_place) {
    ...
    for (int i = 0; i < 3; i++)
      res.board_idx[place_included[g_place][i]] =
          put_arr[this->player][res.board_idx[place_included[g_place][i]]]
                 [local_place[g_place][i]];
    if (place_included[g_place][3] != -1)
      res.board_idx[place_included[g_place][3]] =
          put_arr[this->player][res.board_idx[place_included[g_place][3]]]
                 [local_place[g_place][3]];
    res.player = 3 - this->player;
    return res;
  }
...
private:
  ...
  inline void move_p(board *res, int g_place, int i) {
    ...
    for (j = 1;
         j <= move_arr[this->player]
                      [this->board_idx[place_included[g_place][i]]][place][0];
         ++j)
      flip(res, g_place - global_move[place_included[g_place][i]] * j);
    for (j = 1;
         j <= move_arr[this->player]
                      [this->board_idx[place_included[g_place][i]]][place][1];
         ++j)
      flip(res, g_place + global_move[place_included[g_place][i]] * j);
  }
};
